;	PAGE 67,132
;TITLE	ROMBIOS  - ROM BIOS FOR PC XT
						;COMPARE OK
	INCLUDE	ROMEQU.ASM
...
;----------------------------------------
	...
	ORG	0E05BH
RESET	LABEL	FAR			; FROM POWER_ON_RESET
...
;----- DETERMINE MEMORY SIZE AND FILL MEMORY WITH DATA
	...
	MOV	BX,DATA_WORD[RESET_FLAG-DATA_BASE]
					; SAVE 'RESET_FLAG' IN BX
	MOV	CX,2000H		; SET FOR 16K WORDS
	CMP	BX,WORD PTR MAGIC	; WARM START?
	JE	CLR_STG
	MOV	SP,OFFSET C2
	JMP	STGTST_CNT
C24:	JE	HOW_BIG			; STORAGE OKAY, DETERMINE SIZE
...
HOW_BIG:
	MOV	DATA_WORD[RESET_FLAG-DATA_BASE],BX
					; RESTORE RESET_FLAG
...
FILL_LOOP:
	...
	JNZ	FILL_LOOP
HOW_BIG_END:
	MOV	DATA_WORD[MEMORY_SIZE-DATA_BASE],BX
					; SAVE MEMORY SIZE
...
D3:	MOV	AX,OFFSET D11		; MOVE ADDR OF INTR PROC TO TABLE
	STOSW
...
;------------------------------------------------
	...
	SUB	AH,AH
	MOV	DATA_WORD[EQUIP_FLAG-DATA_BASE],AX
					; SAVE SWITCH INFO
	MOV	AL,99H
	OUT	CMD_PORT,AL
...
D3B:	MOV	AL,38H
	...
	JNZ	E6
	INC	DATA_AREA[MFG_TST-DATA_BASE]
...
;--------------------------------------------------------
E6:
	MOV	AX,DATA_WORD[EQUIP_FLAG-DATA_BASE]
	...
	MOV	DATA_WORD[EQUIP_FLAG-DATA_BASE],AX ;RESTORE IT
					; AND CONTINUE
	AND	AL,30H			; ISOLATE VIDEO SWS
	JNZ	E7			; VIDEO SWS SET TO 0?
	MOV	DI,OFFSET VIDEO_INT	; SET INT10H TO DUMMY
...
E9:					; TEST_VIDEO_STG:
	...
	CMP	DATA_WORD[RESET_FLAG-DATA_BASE],WORD PTR MAGIC
...
;----- TEST THE IMR REGISTER
	...
	MOV	DATA_AREA[MFG_ERR_FLAG-DATA_BASE],05H
					; ************************
					; ****  CHECKPOINT 5  ****
					; ************************
...
;----- INTERRUPTS ARE MASKED OFF.  CHECK THAT NO INTERRUPTS OCCUR.
	...
	MOV	DATA_AREA[INTR_FLAG-DATA_BASE],AL
...
D5:
	LOOP	D5
	CMP	DATA_AREA[INTR_FLAG-DATA_BASE],00H
...
;--------------------------------------------------------
D7:
	MOV	DATA_AREA[MFG_ERR_FLAG-DATA_BASE],02H
					; ********************************
					; ****  TIMER CHECKPOINT (2)  ****
					; ********************************
...
D8:
	TEST	DATA_AREA[INTR_FLAG-DATA_BASE],01H
...
;------------------------------------------------
	...
	OUT	CMD_PORT,AL
	MOV	AL,DATA_AREA[EQUIP_FLAG-DATA_BASE]
	AND	AL,01			; TEST CHAMBER?
	JZ	F7			; BYPASS IF SO
	CMP	DATA_AREA[MFG_TST-DATA_BASE],1
...
;----- SETUP TIMER 0 TO BLINK LED IF MANUFACTURING TEST MODE
	...
	CMP	DATA_AREA[MFG_TST-DATA_BASE],01H ; MFG TEST MODE?
	JNZ	EXP_IO
	MOV	WORD PTR DS:[1CH*4],OFFSET BLINK_INT; SETUP TIMER INTR
 					    ;  TO BLINK LED
...
E19:
	CALL	DDS
	PUSH	DS
	CMP	RESET_FLAG,WORD PTR MAGIC	; WARM START?
...
F16:					; PRT_BASE:
	...
	MOV	PRINTER_BASE-DATA_BASE[SI],DX
...
F17:
	...
	TEST	AL,0F8H
	JNZ	F18
	MOV	RS232_BASE-DATA_BASE[BX],3F8H ; SETUP RS232 CD #1 ADDR
	INC	BX
	INC	BX
	F18:
	...
	TEST	AL,0F8H
	JNZ	F19			; BASE_END
	MOV	RS232_BASE-DATA_BASE[BX],2F8H ; SETUP RS232 CD #2 ADDR
	INC	BX
	INC	BX
...
ROM_ERR	ENDP
	...
	DB	3 DUP(0FFH)	;DUMMY *******
	SUBTTL INT 19H ROUTINE (BOOT STRAP LOADER)
	PAGE
;----- INT 19 ------------------------------------------------
; BOOT STRAP LOADER
...
H4:
	JMP	BOOT_LOCN	;GO ON WITH BOOT RECORD JUST READ IN
BOOT_STRAP	ENDP
	DB	3 DUP(0FFH)	;DUMMY *************************
	SUBTTL INT 14H ROUTINE (RS232 I/O)
	PAGE
;----- INT 14 ---------------------------------------------------------
; RS232_IO
...
;----------------------------------------------------------------------
	...
	ORG	0E729H
A1	LABEL	WORD		;TABLE OF INIT VALUES
	DW	1047		;110 BAUD
	DW	768		;150
	DW	384		;300
	DW	192		;600
...
;------ VECTOR FOR APPROPRIATE ROUTINE
	...
	CALL	DDS
	MOV	DX,[SI]		;GET RS232_BASE ADDRESS
...
;------ DETERMINE BAUD RATE DIVISOR
	...
	MOV	DX,[SI]		;POINT TO HIGH ORDER OF DIVISOR
				;RS232_BASE[SI]
	INC	DX
...
A12:
	...
	MOV	DX,[SI]		;DATA PORT, RS232_BASE[SI]
...
F3D	DB	'ERROR. (RESUME = "F1" KEY)',13,10 ;ERROR PROMPT
	DB	9 DUP(0FFH)	;DUMMY ***************************
	SUBTTL INT 16H ROUTINE (KEYBOARD I/O)
	PAGE
;---- INT 16 -----------------------------------------------------
; KEYBOARD I/O
...
;----- CTL-ALT-DEL HAS BEEN FOUND, DO I/O CLEANUP
	...
	MOV	RESET_FLAG, WORD PTR MAGIC  ;SET FLAG FOR RESET FUNCTION
...
K40:				; PAUSE-LOOP
	TEST	KB_FLAG_1,HOLD_STATE ; LOOP UNTIL FLAG TURNED OFF
...
K42:				; NOT-KEY-55
	MOV	BX,OFFSET K8	; SET UP TRANSLATE CTL
...
F3	DB	'601',13,10	;DISKETTE ERROR
	DB	2 DUP(0FFH)	;DUMMY **************************
	SUBTTL INT 13H ROUTINE (DISKETTE I/O)
	PAGE
;-- INT 13 --------------------------------------------------
; DISKETTE I/O
...
J9:				;DISK_READ_CONT
	...
	JMP	SHORT RW_OPN	;GO TO THE OPERATION
DISK_READ	ENDP
...
;-------------- TURN ON THE MOTOR AND SELECT DRIVE
	...
	TEST	AL,MOTOR_STATUS	;TEST THAT MOTOR FOR OPERATION
...
J22:				;OPN_OK
	...
	RET
RW_OPN	ENDP
	SUBTTL SUBROUTINES FOR INT 13H ROUTINE (DISKETTE I/O)
	PAGE
;---------------------------------------------------------------------
; NEC_OUTPUT
...
DISK_BASE	LABEL	BYTE
	...
	SUBTTL INT 17H ROUTINE (PRINTER I/O)
	PAGE
;------- INT 17 -------------------------------------------------
; PRINTER I/O
...
PRINTER_IO PROC	FAR
	...
	MOV	BL,[SI+78H]	;LOAD PRINT_TIM_OUT PARM
	SHL	SI,1		; WORD OFFSET INTO TABLE
	MOV	DX,[SI+08H]	;GET BASE ADDRESS FOR PRINTER CARD
				;PRINTER_BASE[SI]
...
B9:				; INIT_LOOP
	...
	JMP	B6		; PRT_STATUS_1
PRINTER_IO	ENDP
	DB	0FFH,0FFH,0FFH,0FFH	;DUMMY **************
	SUBTTL INT 10H ROUTINE (VIDEO I/O)
	PAGE
;--- INT 10 -------------------------------------------------------------
; VIDEO_IO 								:
...
M4	EQU	$-VIDEO_PARMS
	...
	DB	0,0,0,0
M5	LABEL	WORD		;TABLE OF REGEN LENGTH
	DW	2048		;40X25
	DW	4096		;80X25
	DW	16384		;GRAPHICS
	DW	16384
...
ACT_DISP_PAGE	PROC	NEAR
	...
	MOV	AX,[BX + CURSOR_POSN-DATA_BASE]
...
SET_COLOR	PROC	NEAR
	...
	ADD	DX,5			; GET THE CURRENT PALETTE VALUE
...
;----- HANDLE COLOR 1 BY SETTING THE PALETTE TO BE USED
...
WRITE_DOT	PROC	NEAR
	...
	OR	AL,CL			; OR THE NEW VALUE OF THOSE BITS
R1:					; FINISH_DOT
...
;----- DETERMINE BIT OFFSET IN BYTE FOR COLUMN MASK
...
;----- MULTIPLY DH (VALID BITS IN BYTE) BY CH (BIT OFFSET)
...
;	 BOTH OF THE ABOVE ARE IN CHARACTER POSITION		:
...
;  SUPPLIED TABLE OF GRAPHICS IMAGES (8X8 BOXES).		:
...
;----- DETERMINE REGIONS TO GET CODE POINTS FROM

	POP	AX			; RECOVER CODE POINT
	CMP	AL,80H			; IS IT IN SECOND HALF ?
	JAE	S1			; YES

;----- IMAGE IT IN FIRST HALF, CONTAINED IN ROM
...
GRAPHICS_READ	PROC	NEAR
	CALL	S26			; CONVERT TO OFFSET IN REGEN
...
;----- SAVE AREA HAS CHARACTER IN TI, MATCH IT
...
S17:
	...
	POP	SI
	JZ	S18			; IF ZERO FLAG SET, THEN MATCH OCCURED
...
S19	PROC	NEAR
	...
	PUSH	CX			; SAVE REGISTER
	MOV	CX,3			; NUMBER OF TIMES TO DO IT
S20:
	SAL	AL,1
...
S23	PROC	NEAR
	...
	MOV	CX,0C000H		; 2 MASK BITS TO TEST THE ENTRIES
...
S25:	RCL	DL,1			; MOVE THAT BIT INTO THE RESULT
	SHR	CX,1
	SHR	CX,1			; MOVE THE MASK TO THE RIGHT 2 BITS
...
GRAPH_POSN	LABEL	NEAR
	...
	SHL	AX,1			; MULTIPLY BY 4 SINCE 4 ROWS/BYTE
	SHL	AX,1
...
;----- AX HAS THE VALUE READ IN FROM THE 6845
        ...
        MOV	BL,CS:V1[BX]		; DETERMINE AMOUNT OF SUBTRACT
...
;----- GRAPHICS MODE
        ...
        DIV	DL			;  AL RANGE 0-99, AH RANGE 0-39
...
V3:					; NOT_HIGH_RES
        SHL	BX,CL			; MULTIPLY BY 16 FOR HIGH RES
...
READ_LPEN	ENDP
	DB	7 DUP(0FFH)	;DUMMY **************************
	SUBTTL INT 12H ROUTINE (MEMORY SIZE DETERMINATION)
	PAGE
;--- INT 12 -------------------------------------------------------------
; MEMORY_SIZE_DET							:
...
MEMORY_SIZE_DET	ENDP
	DB	2 DUP(0FFH)	;DUMMY **************************
	SUBTTL INT 11H ROUTINE (EQUIPMENT DETERMINATION)
	PAGE
;--- INT 11 -----------------------------------------------------
...
EQUIPMENT	ENDP
	...
	DB	2 DUP(0FFH)	;DUMMY **************************
;--- INT 15 -------------------------------------------------------------
;	DUMMY CASSETTE IO ROUTINE-RETURNS 'INVALID CMD' IF THE ROUTINE	:
...
CASSETTE_IO	PROC	FAR
	...
	RET	2
CASSETTE_IO	ENDP
	SUBTTL NON-MASKABLE INTERRUPT ROUTINE
	PAGE
;--- INT 2 -----------------------------------------------------------
...
F4	LABEL	WORD			; PRINTER SOURCE TABLE
	...
	DW	378H
	DW	278H
F4E	LABEL	WORD			; PRINTER SOURCE TABLE
...
G12A:
	...
	RET
P_MSG	ENDP
	SUBTTL INITIAL RELIABILITY TEST -- SUBROUTINES
	PAGE
;-------------------------------------------
...
DDS	ENDP
	...
	DB	13 DUP(0FFH)	;DUMMY **************************
	SUBTTL CHARACTER GENERATOR GRAPHICS
	PAGE
;------------------------------------------------------------------------
...
CRT_CHAR_GEN	LABEL	BYTE
	...
	SUBTTL INT 1AH ROUTINE (TIME OF DAY)
	PAGE
;--- INT 1A ---------------------------------------------
; TIME_OF_DAY						:
...
;	DW	0F600H			;  MUST BE INSERTED INTO TABLE LATER
	...
	DW	OFFSET DISK_BASE	; INTERRUPT 1EH -- DISK PARAMETERS
...
;------------------------------------------------
	ORG	0FF53H
	IRET
	SUBTTL INT 05H ROUTINE (PRINT SCREEN)
	PAGE
;-- INT 5 ---------------------------------------------------------------
; PRINT_SCREEN
